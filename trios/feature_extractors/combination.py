...
def extract_pattern(combinationPattern, img, i, j, pat):
    ...
    n = len(combinationPattern)
    for k in range(n):
        pat[k] = 0
    for k in range(n):
        cls = combinationPattern.wops[k].classifier
        fe = combinationPattern.wops[k].extractor
        fe.extract(img, i, j, combinationPattern.fvectors[k])
        lbl = cls.apply(combinationPattern.fvectors[k])
        if combinationPattern.bitpack == False:
            pat[k] = lbl
        else:
            sh = int(k / 32)
            byt = k % 32
...
class CombinationPattern(FeatureExtractor):
  ...
  publisher={IEEE}
    ...
    def extract_image(self,inp,  msk, _X,  k):
        X = _X
        img = []
        i, j, l, sh, byt, ww2, hh2 = 0
        imgsize = inp.shape[0] * inp.shape[1]
